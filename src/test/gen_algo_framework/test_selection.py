...
from src.gen_algo_framework.selection import cumulative_list, roulette_wheel_selection_two_parents, roulette_wheel_toss


def __random_c_list():
    _population = [None] * randint(20, 60)
    _population = list(map(lambda x: (float(randint(1 , 1000)), x), _population))
    return cumulative_list(_population)


def test_cumulative_list():
    for _ in range(5000):
        c_list = __random_c_list()
        ...
        assert 0.99 < c_list[-1] and c_list[-1] < 1.00001, f'Last element out of range: {c_list[-1]}'
